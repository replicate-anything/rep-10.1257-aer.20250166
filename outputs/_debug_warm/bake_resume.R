# Resume Hahn bake after fig_1 alias fix. only_missing=TRUE skips done artifacts.
# Use only_missing=FALSE when intentional full retime is needed.
...
only_missing <- TRUE
if (length(commandArgs(trailingOnly = TRUE)) && commandArgs(trailingOnly = TRUE)[[1]] == "all") {
  only_missing <- FALSE
}

cat("=== bake resume", format(Sys.time(), "%Y-%m-%d %H:%M:%S"), "===\n")
cat("stata_timeout=", getOption("replicateEverything.stata_timeout"), "\n")
cat("only_missing=", only_missing, "\n")
...
err <- NULL
result <- tryCatch(
  build_study_outputs(study, install_deps = TRUE, only_missing = only_missing),
  error = function(e) {
...
lines <- c(
  ...
  paste0("only_missing: ", only_missing),
...
if (!is.null(timings$steps)) {
  ...
  for (nm in names(timings$steps)) {
    ...
    sec <- step$seconds %||% step$elapsed %||% step
