...
preserve
	...
	if "`savings'" == "no" {
		collapse (mean) WTP program_cost cost env_cost_wtp cost_wtp producers wtp_env fisc_tot transfer table_order, by(group_label)
	}	
	
	if "`savings'" == "yes" {
		collapse (mean) WTP program_cost cost env_cost_wtp cost_wtp producers wtp_env fisc_tot transfer c_savings table_order, by(group_label)
	}
...
**************************
keep program_label_short group_label MVPF WTP cost env_cost_wtp cost_wtp producers wtp_env fisc_tot transfer table_order c_savings across_group_ordering
sort table_order
