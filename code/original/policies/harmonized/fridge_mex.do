...
****************************************************
	...
	*431 gCO2/kWh from: https://www.climate-transparency.org/wp-content/uploads/2021/10/CT2021Mexico.pdf 
	...
	local adj_subsidy = 129400000 / 858962 * (${cpi_`dollar_year'}/${cpi_2012}) // total spent on program divided by number of fridge replacements
			
	rebound ${rebound}
	local r = `r(r)'
	
	local kwh_change = `kwh_reduced' * 12 // Converting monthly change to annual change
	
*************************
/* 4. WTP Calculations */
*************************
local prop_marginal = 0.5 // We are assuming that 50% of recipients are marginal
