...
********************************************************************************
/*
Jayachandran, Seema, Joost de Laat, Eric F. Lambin, Charlotte Y. Stanton, Robin Audy, Nancy E. Thomas.
...
local admin_cost_raw = 14 + 30 +  0.1 * `subsidy_raw' // per eligible PFO, from supplemental material section 4, 14 = monitoring with 2 checks a day, 30 = marketing + program management, 10% of subsidy = transaction fee
local subsidy = `subsidy_raw' * (${cpi_`dollar_year'} / ${cpi_${policy_year}})
local admin_cost = `admin_cost_raw' * (${cpi_`dollar_year'} / ${cpi_${policy_year}})

local program_cost = `subsidy' + `admin_cost_raw' * (${cpi_`dollar_year'} / ${cpi_${policy_year}})

local treatment_tree_loss = 0.095 - `treatment_effect' // 4.3 log points
local treatment_loss_percent = exp(`treatment_tree_loss') - 1 // 4.2% tree loss in treatment group

local control_loss_percent = -(exp(-0.095) - 1) // 9.1% tree loss in control group
...
local wtp_soc = `carbon_per_pfo' * `scc' * (1 - (${USShareFutureSSC} * ${USShareGovtFutureSCC}))

local fiscal_externality_lr = -`carbon_per_pfo' * `scc' * (${USShareFutureSSC} * ${USShareGovtFutureSCC})
...
local WTP_USPres = 0	
local WTP_USFut = (`carbon_per_pfo' * `scc') * (${USShareFutureSSC} - (${USShareFutureSSC} * ${USShareGovtFutureSCC}))
local WTP_RoW = ((1 - ${USShareFutureSSC}) * (`carbon_per_pfo' * `scc')) + `wtp_marg' + `wtp_infr'
...
global `1'_name "Payments for Ecosystem Services to Reduce Deforestation in Uganda"
